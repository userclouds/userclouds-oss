...
// copied in from crewjam/saml for testing reasons
...
func TestSPCanSetAuthenticationNameIDFormat(t *testing.T) {
	...
	req, err := s.MakeAuthenticationRequest("", samlconfig.HTTPRedirectBinding, samlconfig.HTTPPostBinding)
	assert.Check(t, err)
...
func TestSPCanProduceMetadataWithEncryptionCert(t *testing.T) {
	...
	s := ServiceProvider{
		...
		LogoutBindings: []string{samlconfig.HTTPPostBinding},
	}
...
func TestSPCanProduceMetadataWithBothLougoutBindings(t *testing.T) {
	...
	s := ServiceProvider{
		...
		SloURL:         mustParseURL("https://example.com/saml2/slo"),
		IDPMetadata:    &samlconfig.EntityDescriptor{},
		LogoutBindings: []string{samlconfig.HTTPPostBinding, samlconfig.HTTPRedirectBinding},
	}
	err := xml.Unmarshal(test.IDPMetadata, &s.IDPMetadata)
	assert.Check(t, err)
...
func TestSPCanProduceRedirectRequest(t *testing.T) {
	test := NewServiceProviderTest(t)
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05.999999999 UTC 2006", "Mon Dec 1 01:31:21.123456789 UTC 2015")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestSPCanProducePostRequest(t *testing.T) {
	test := NewServiceProviderTest(t)
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Mon Dec 1 01:31:21 UTC 2015")
		assert.NilError(t, err)
		return rv
	}
	s := ServiceProvider{
...
func TestSPCanHandleOneloginResponse(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Jan 5 17:53:12 UTC 2016")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
	...
	assert.Check(t, is.DeepEqual([]saml.Attribute{
		{
			Name:       "User.email",
...
func TestSPCanHandleOktaSignedResponseEncryptedAssertion(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Mar 3 19:24:28 UTC 2020")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestSPCanHandleOktaResponseEncryptedSignedAssertion(t *testing.T) {
	...
	// An actual response from okta - captured with trivial.go + test.Key/test.Certificate
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Mar 3 19:31:55 UTC 2020")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestSPCanHandleOktaResponseEncryptedAssertionBothSigned(t *testing.T) {
	...
	// An actual response from okta - captured with trivial.go + test.Key/test.Certificate
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Mar 3 19:40:54 UTC 2020")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestSPCanHandlePlaintextResponse(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 UTC 2006", "Tue Jan 5 16:55:39 UTC 2016")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestSPRejectsInjectedComment(t *testing.T) {
	...
	{
		x, err := base64.StdEncoding.DecodeString(string(SamlResponse))
		assert.NilError(t, err)
	...
	// it *MUST NOT* validate
	{
		...
		assert.Check(t, errors.As(err, &ire))
		realErr := ire.PrivateErr
		assert.Check(t, realErr != nil, strings.Contains(realErr.Error(),
...
func TestSPCanParseResponse(t *testing.T) {
	...
	assert.Check(t, is.DeepEqual([]saml.Attribute{
		...
		},
		{
			...
			Values: []saml.AttributeValue{
				{
					NameID: &saml.NameID{Format: "urn:oasis:names:tc:SAML:2.0:nameid-format:persistent", NameQualifier: "https://idp.testshib.org/idp/shibboleth", SPNameQualifier: "https://15661444.ngrok.io/saml2/metadata", Value: "8F+M9ovyaYNwCId0pVkVsnZYRDo="},
				},
			},
		},
		{
...
func TestServiceProviderMismatchedDestinationsWithSignaturePresent(t *testing.T) {
	...
	bs, err := test.responseDom(t).WriteToBytes()
	assert.NilError(t, err)
	req.PostForm.Set("SAMLResponse", base64.StdEncoding.EncodeToString(bs))
	_, err = s.ParseResponse(&req, []string{"id-9e61753d64e928af5a7a341a97f420c9"})
	var ire *InvalidResponseError
	assert.Check(t, errors.As(err, &ire))
	assert.Check(t, err != nil, strings.Contains(ire.PrivateErr.Error(),
...
func TestServiceProviderMissingDestinationWithSignaturePresent(t *testing.T) {
	...
	req := http.Request{PostForm: url.Values{}}
	bs, err := removeDestinationFromDocument(addSignatureToDocument(test.responseDom(t))).WriteToBytes()
	assert.NilError(t, err)
	req.PostForm.Set("SAMLResponse", base64.StdEncoding.EncodeToString(bs))
	_, err = s.ParseResponse(&req, []string{"id-9e61753d64e928af5a7a341a97f420c9"})
	var ire *InvalidResponseError
...
func TestSPMismatchedDestinationsWithSignaturePresent(t *testing.T) {
	...
	bs, err := addSignatureToDocument(test.responseDom(t)).WriteToBytes()
	assert.NilError(t, err)
	req.PostForm.Set("SAMLResponse", base64.StdEncoding.EncodeToString(bs))
	_, err = s.ParseResponse(&req, []string{"id-9e61753d64e928af5a7a341a97f420c9"})
	var ire *InvalidResponseError
...
func TestSPInvalidAssertions(t *testing.T) {
	...
	assert.Check(t, err != nil, strings.Contains(err.Error(), "expired on 2015-12-01 01:57:51.375 +0000 UTC"))

	assertion.Issuer.Value = "bob"
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "issuer is not \"https://idp.testshib.org/idp/shibboleth\""))
	assertion = saml.Assertion{}
	...
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion SubjectConfirmation one of the possible request IDs ([any request id])"))

	assertion.Subject.SubjectConfirmations[0].SubjectConfirmationData.Recipient = "wrong/acs/url"
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion SubjectConfirmation Recipient is not https://15661444.ngrok.io/saml2/acs"))
	assertion = saml.Assertion{}
	assert.Check(t, xml.Unmarshal(assertionBuf, &assertion))

	assertion.Subject.SubjectConfirmations[0].SubjectConfirmationData.NotOnOrAfter = TimeNow().Add(-1 * time.Hour)
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion SubjectConfirmationData is expired"))
	assertion = saml.Assertion{}
	assert.Check(t, xml.Unmarshal(assertionBuf, &assertion))

	assertion.Conditions.NotBefore = TimeNow().Add(time.Hour)
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion Conditions is not yet valid"))
	assertion = saml.Assertion{}
	assert.Check(t, xml.Unmarshal(assertionBuf, &assertion))

	assertion.Conditions.NotOnOrAfter = TimeNow().Add(-1 * time.Hour)
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion Conditions is expired"))
	assertion = saml.Assertion{}
	assert.Check(t, xml.Unmarshal(assertionBuf, &assertion))

	assertion.Conditions.AudienceRestrictions[0].Audience.Value = "not/our/metadata/url"
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err != nil, strings.Contains(err.Error(), "assertion Conditions AudienceRestriction does not contain \"https://15661444.ngrok.io/saml2/metadata\""))
	assertion = saml.Assertion{}
	assert.Check(t, xml.Unmarshal(assertionBuf, &assertion))

	// Not having an audience is not an error
	assertion.Conditions.AudienceRestrictions = []saml.AudienceRestriction{}
	err = s.validateAssertion(&assertion, []string{"id-9e61753d64e928af5a7a341a97f420c9"}, TimeNow())
	assert.Check(t, err)
}
...
func TestXswPermutationThreeIsRejected(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse(timeFormat, "2014-07-17T01:02:59Z")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
	...
	var ire *InvalidResponseError
	assert.Check(t, errors.As(err, &ire))
	assert.Check(t, err != nil, strings.Contains(ire.PrivateErr.Error(), "signature element not present"))
}
...
func TestXswPermutationSevenIsRejected(t *testing.T) {
	...
	Clock = dsig.NewFakeClockAt(func() time.Time {
		rv, err := time.Parse(timeFormat, "2014-07-17T14:12:57Z")
		assert.NilError(t, err)
		return rv
	}())
...
func TestSPRealWorldKeyInfoHasRSAPublicKeyNotX509Cert(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse("Mon Jan 2 15:04:05 MST 2006", "Fri Apr 21 13:12:51 UTC 2017")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestGetArtifactBindingLocation(t *testing.T) {
	...
	location := sp.GetArtifactBindingLocation(samlconfig.SOAPBinding)
...
func TestParseXMLArtifactResponse(t *testing.T) {
	test := NewServiceProviderTest(t)
	TimeNow = func() time.Time {
		rv, err := time.Parse(timeFormat, "2021-08-17T10:26:57Z")
		assert.NilError(t, err)
		return rv
	}
	Clock = dsig.NewFakeClockAt(TimeNow())
...
func TestParseBadXMLArtifactResponse(t *testing.T) {
	...
	TimeNow = func() time.Time {
		rv, err := time.Parse(timeFormat, "2022-08-17T10:26:57Z")
		assert.NilError(t, err)
		return rv
	}
	...
	Clock = dsig.NewFakeClockAt(func() time.Time {
		rv, err := time.Parse(timeFormat, "2039-08-17T10:26:57Z")
		assert.NilError(t, err)
		return rv
	}())
	TimeNow = func() time.Time {
