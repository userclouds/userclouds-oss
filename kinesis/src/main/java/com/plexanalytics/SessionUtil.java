...
public class SessionUtil {
	...
	public static boolean checkIfStreamExist(String region, String streamName) {
		...
		try {
			...
			e.printStackTrace();
			log.error("Exception received while listing shards for stream: " + streamName);
		}
		kinesis.shutdown();
		return streamExist;
	}
	...
	public static boolean checkIfBucketExist(String region, String s3Path) {
		...
		String prefix = "s3://"; // To use in starts with
		String bktname = s3Path.substring(5);
		...
		try {
			if (s3Path.startsWith(prefix)) {
				if (s3.doesBucketExistV2(bktname)) {
					...
					log.error("Didn't find bucket: " + region + " " + bktname);
				}
			} else {
				log.error("didn't match the prefix: " + s3Path + " " + prefix);
			}
		} catch (Exception e) {
			e.printStackTrace();
