...
func init() {
	tc, err := NewServerTLSConfig(test_keys.CaPem, test_keys.CertPem, test_keys.KeyPem, tls.VerifyClientCertIfGiven)
	if err != nil {
		panic(err)
	}
	tlsConf = tc
...
func prepareServerConf(t *testing.T) []*Server {
...
func (s *serverTestSuite) TearDownSuite() {
	if s.db != nil {
		assert.NoErr(s.t, s.db.Close())
	}

	if s.l != nil {
		assert.NoErr(s.t, s.l.Close())
	}
}
...
func (s *serverTestSuite) onConn(conn net.Conn) {
	...
	co, err := NewCustomizedConn(ctx, conn, s.server, s.credProvider, &testHandler{s})
	require.NoError(s.T(), err)
	// set SSL if defined
	for {
		err = co.HandleCommand(ctx)
		if err != nil {
			return
		}
	}
...
func (h *testHandler) handleQuery(query string, binary bool) (*mysql.Result, error) {
	...
	default:
		return nil, ucerr.Errorf("invalid query %s", query)
	}
}
...
func (h *testHandler) HandleStmtPrepare(sql string) (params int, columns int, ctx any, err error) {
	...
	case "delete":
		...
		columns = 0
	default:
		err = ucerr.Errorf("invalid prepare %s", sql)
	}
...
func (h *testHandler) HandleOtherCommand(cmd byte, data []byte) error {
	return ucerr.Wrap(mysql.NewError(mysql.ER_UNKNOWN_ERROR, fmt.Sprintf("command %d is not supported now", cmd)))
}
