package testkeys

import (
	"userclouds.com/infra/secret"
)

// CertificatePrivateKeyPEMSecret is just a secret.String wrapping the cert PK for tests
var CertificatePrivateKeyPEMSecret = secret.NewTestString(CertificatePrivateKeyPEM)

// CertificatePrivateKeyPEM is a PEM-encoded X.509 certificate for tests
const CertificatePrivateKeyPEM = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`

// CertificatePEM is the PEM-encoded X.509 certificate for tests
const CertificatePEM = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
