@authz
@authz_objects
Feature: authz objects table

  @a11y
  Scenario: empty authz objects list accessibility
  ...
  Scenario: empty authz objects list
    Given I am a logged-in user
    And a mocked "GET" request for "tenants"
    And a mocked "GET" request for "selectedTenant"
    And the following mocked requests:
  ...
  Scenario: authz objects list
    ...
    And I should see a button labeled "Create Object"
  ...
  Scenario: authz objects delete
    ...
    When I toggle the checkbox in column 1 of row 2 of the table with ID "objectsTable"
    Then row 2 of the table with ID "objectsTable" should not be marked for delete
    ...
    Then I should see the following text within the dialog titled "Delete Objects"
      | Selector | Text                                                                   |
      | div      | Are you sure you want to delete 1 object? This action is irreversible. |
    When I click the button with ID "confirmDeleteButton"
    Then I should not see an element matching selector "#objectsTable tbody > tr:first-child[class*='queuedfordelete']"

  Scenario: authz objects filter
    Given I am a logged-in user
    And a mocked "GET" request for "tenants"
    And a mocked "GET" request for "selectedTenant"
    And the following mocked requests:
    ...
    When I click the button labeled "Add"
    ...
    And I select the option labeled "Filter by Alias" in the dropdown matching selector "select#objectscolumns"
    ...
    When I select the option labeled "Filter by Created" in the dropdown matching selector "select#objectscolumns"
  ...
  Scenario: Non-Admin AuthZ Page
    ...
    When I navigate to the page with path "/objects?company_id=1ee4497e-c326-4068-94ed-3dcdaaaa53bc&tenant_id=41ab79a8-0dff-418e-9d42-e1694469120a"
    Then the page title should be "[dev] UserClouds Console"
    And I should not see an element matching selector "#authZObjects button:has-text('Create Object')"
    And I should not see an element matching selector "#authZObjects button:has-text('Create Object Type')"
    And I should not see an element matching selector "#authZObjects button:has-text('Create Edge')"
    And I should not see an element matching selector "#authZObjects button:has-text('Create Edge Type')"
    And I should not see an element matching selector "#authZObjects button:has-text('Edit')"
