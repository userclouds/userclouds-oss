package auth

import (
	"context"
	"log"
	"net/url"
	"os"

	"github.com/coreos/go-oidc/v3/oidc"
	"golang.org/x/oauth2"
)

// Authenticator abstracts different types of Authenticators
type Authenticator struct {
	Provider *oidc.Provider
	Config   oauth2.Config
}

// Const string enum for which IDP to use in the sample events app.
const (
	ModeAuth0 = "Auth0"
	ModePlex  = "Plex"
)

// GetMode returns one of the defined modes to indicate which IDP the
// sample events app is using.
func GetMode() string {
	env := os.Getenv("AUTH_MODE")
	if env != ModePlex && env != ModeAuth0 {
		log.Fatalf("Invalid AUTH_MODE env var set: %s", env)
	}
	return env
}

// UseAuth0 returns true if the events app is configured to use Auth0,
// and false if configured to use Plex (UserClouds).
func UseAuth0() bool {
	return GetMode() == ModeAuth0
}

// NewAuthenticator returns an Authenticator based on env vars
func NewAuthenticator() (*Authenticator, error) {
	if UseAuth0() {
		return newAuth0Authenticator()
	}
	return newUserCloudsAuthenticator()
}

// NewAltAuthenticator returns the alternate authenticator object (opposite of NewAuthenticator())
func NewAltAuthenticator() (*Authenticator, error) {
	if UseAuth0() {
		return newUserCloudsAuthenticator()
	}
	return newAuth0Authenticator()
}

// GetLogoutURL returns the (mode-dependent) URL to redirect to in order to log the user out.
func GetLogoutURL(clientID, redirectURL string) string {
	query := url.Values{}
	query.Add("client_id", clientID)

	if UseAuth0() {
		query.Add("returnTo", redirectURL)
		return "https://" + os.Getenv("AUTH0_DOMAIN") + "/v2/logout?" + query.Encode()
	}

	query.Add("redirect_url", redirectURL)
	return os.Getenv("UC_TENANT_BASE_URL") + "/logout?" + query.Encode()
}

// GetClientID returns the (mode-dependent) client ID for the IDP.
func GetClientID() string {
	if UseAuth0() {
		return os.Getenv("AUTH0_CLIENT_ID")
	}
	return os.Getenv("PLEX_CLIENT_ID")
}

func newAuth0Authenticator() (*Authenticator, error) {
	provider, err := oidc.NewProvider(context.TODO(), "https://"+os.Getenv("AUTH0_DOMAIN")+"/")
	if err != nil {
		log.Printf("failed to get provider: %v", err)
		return nil, err
	}

	conf := oauth2.Config{
		ClientID:     os.Getenv("AUTH0_CLIENT_ID"),
		ClientSecret: os.Getenv("AUTH0_CLIENT_SECRET"),
		RedirectURL:  os.Getenv("OIDC_CALLBACK_URL"),
		Endpoint:     provider.Endpoint(),
		Scopes:       []string{oidc.ScopeOpenID, "profile", "email"},
	}

	return &Authenticator{
		Provider: provider,
		Config:   conf,
	}, nil
}

func newUserCloudsAuthenticator() (*Authenticator, error) {
	provider, err := oidc.NewProvider(context.TODO(), os.Getenv("UC_TENANT_BASE_URL"))
	if err != nil {
		log.Printf("failed to get provider: %v", err)
		return nil, err
	}

	conf := oauth2.Config{
		ClientID:     os.Getenv("PLEX_CLIENT_ID"),
		ClientSecret: os.Getenv("PLEX_CLIENT_SECRET"),
		RedirectURL:  os.Getenv("OIDC_CALLBACK_URL"),
		Endpoint:     provider.Endpoint(),
		Scopes:       []string{oidc.ScopeOpenID, "profile", "email"},
	}

	return &Authenticator{
		Provider: provider,
		Config:   conf,
	}, nil
}
